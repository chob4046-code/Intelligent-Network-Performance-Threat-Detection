...
def login():
    ...
    username=(request.form.get('username') or '').strip(); password=request.form.get('password') or ''; ip=request.remote_addr or 'unknown'; key=client_key(ip); current=now()
    if login_failures(Config.DB_PATH,key,current-300)>=5:
    ...
    user=get_user(Config.DB_PATH,username); success=bool(user and verify_password(password,user['password_hash']))
    ...
    session.clear(); session['user_id']=user['id']; session['username']=user['username']; record_event(Config.DB_PATH,'login_success','info',ip,'User authenticated')
    return redirect(url_for('main.index'))

@bp.post('/logout')
@login_required
def logout():
    record_event(Config.DB_PATH,'logout','info',request.remote_addr,'User logged out'); session.clear(); return redirect(url_for('main.login'))

@bp.post('/targets')
@login_required
def create_target():
    name=(body('name') or '').strip(); host=(body('host') or '').strip()
    ...
    target_id=add_target(Config.DB_PATH,name,host,port); record_event(Config.DB_PATH,'target_added','info',request.remote_addr,f'Monitoring target {name}')
    return jsonify({'id':target_id,'message':'target created'}),201

@bp.post('/targets/<int:target_id>/toggle')
@login_required
def toggle(target_id):
    enabled=str(body('enabled','1')).lower() in {'1','true','on'}; set_target_enabled(Config.DB_PATH,target_id,enabled); return jsonify({'enabled':enabled})

@bp.delete('/targets/<int:target_id>')
@login_required
def remove(target_id):
    delete_target(Config.DB_PATH,target_id); record_event(Config.DB_PATH,'target_removed','info',request.remote_addr,f'Target {target_id} removed'); return jsonify({'message':'deleted'})

@bp.post('/alerts/<int:alert_id>/ack')
@login_required
def ack(alert_id):
    acknowledge_alert(Config.DB_PATH,alert_id); return jsonify({'message':'acknowledged'})

@bp.get('/api/overview')
@login_required
def overview():
    ts,ev,al,total,up=dashboard_data(Config.DB_PATH)
...
@login_required
def history(target_id): return jsonify([dict(x) for x in recent_checks(Config.DB_PATH,target_id)])
